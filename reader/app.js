...
(function () {
  ...
  /* ---------- setup: one static page, no questions — read it, run it, go ---------- */
  function startSetup() {
    state = { view: "setup" };
    render();
  }
  ...
  function renderSetup() {
    app.innerHTML = `<div class="setup">
      ...
      <div class="chat">
        <div class="bubble bot">Four commands, once, in a terminal — then every project just runs.</div>
        <div class="bubble bot"><code>git clone https://github.com/MoreSalamander/taichi-academy.git
cd taichi-academy
python3.11 -m venv .venv
source .venv/bin/activate
pip install -e ".[dev]"</code></div>
        <div class="bubble bot">On Windows, the third line is <code>.venv\\Scripts\\activate</code> instead.</div>
        <div class="bubble bot">Needs Python 3.11 specifically — not 3.12, not 3.9. Don't have it? Grab it from
          <a href="https://www.python.org/downloads/release/python-3119/" target="_blank" rel="noopener">python.org</a>,
          then run the four commands above.</div>
        <div class="bubble bot">That's it. Click below and we'll jump straight into chapter 1, step 1.</div>
      </div>
      <div class="setup-controls"><button class="btn primary" id="build">Start Project 01 →</button></div>
    </div>`;
    document.getElementById("home").onclick = () => { state = { view: "menu" }; render(); };
    document.getElementById("build").onclick = () => {
      const first = PROJECTS.find((p) => p.status === "available");
      if (first) openProject(first.id); else { state = { view: "menu" }; render(); }
    };
  }
