"""Code SOT for project 13 — planet generator.

Fragments in FINAL document order; versions keyed by (chapter, step).
Verify with `python tools/build_fulls.py --project 13-planet-generator`.

Evolutions: render() has three versions — bare gray sphere with diffuse
lighting only (chapter 2, proving ray_sphere and the camera work before any
terrain exists), +terrain colors via surface_color (chapter 3), +ocean
specular and atmosphere rim (chapter 4). seed_terrain replaces a plain
fbm3d upload in chapter 3 step 2, introducing the quantile-anchored sea
level. The camera/orbit machinery is carried over from project 12 nearly
verbatim and lands fully-formed in chapter 2.
"""
...
SPEC = FragmentSet(
    project_id="13-planet-generator",
    default_file="planet.py",
    reference={"planet.py": PROJECT_DIR / "reference" / "planet.py"},
    chapter_steps={1: 3, 2: 2, 3: 3, 4: 3},
)
frag = SPEC.frag

# --- module head ---------------------------------------------------------------------

frag(((1, 1), '"""Planet Generator: a whole world from one seed — 3D noise on a sphere, no map seams."""'))
...
frag(((3, 2), "OCEAN_FRACTION = 0.62"))
frag(((3, 2), "SEA_LEVEL = 0.5"))
frag(((3, 2), "ELEV_GAIN = 1.5"))
frag(((3, 3), "ICE_LAT = 0.72"))

frag(((2, 1), "PLANET_R = 1.0"))
frag(((2, 2), "CAM_RADIUS = 2.6"))
frag(((2, 2), "CAM_HEIGHT = 0.5"))
frag(((2, 2), "ORBIT_SPEED = 0.004"))
frag(((2, 2), "ZOOM = 1.8"))

frag(((2, 2), "SUN = (0.6, 0.3, -0.6)"))

# --- module-level fields ------------------------------------------------------------

frag(((1, 2), "elevation = None"))
...
frag(((1, 2), "    elevation = ti.field(ti.f32, shape=(VOL_N, VOL_N, VOL_N))"))
...
FBM3D = '''def fbm3d(n, rng_seed=0, octaves=5, roughness=0.55):
...
SEED_TERRAIN = '''def seed_terrain(n, rng_seed=0):
...
frag(((3, 3), SURFACE_COLOR))

# --- the renderer: three versions -------------------------------------------------------
...
frag(((2, 2), RENDER_V1), ((3, 3), RENDER_V2), ((4, 1), RENDER_V3))
...
frag(((2, 2), CAMERA_POSITION))

# --- main (small sub-fragments) -------------------------------------------------------

frag(((1, 3), "def main():\n    init_sim()\n    apply_seed()"))
frag(((1, 3), '    gui = ti.GUI("Planet Generator — taichi-academy", res=RES, background_color=0x0A0A12)'))
frag(((2, 2), "    theta = 0.0"))
frag(((4, 2), "    pmx = None"))
...
THETA_V1 = "        theta += ORBIT_SPEED"

THETA_V2 = """        if gui.is_pressed(ti.GUI.LMB):
...
frag(((2, 2), THETA_V1), ((4, 2), THETA_V2))

frag(((2, 2), "        cx, cy, cz = camera_position(theta)"))
frag(((2, 2), "        render(cx, cy, cz, *SUN)"))
frag(((1, 3), "        gui.set_image(pixels)"))
frag(((4, 3), '        gui.text("drag to orbit  [r] new planet", (0.02, 0.98), color=0xFFFFFF)'))
frag(((1, 3), "        gui.show()"))
